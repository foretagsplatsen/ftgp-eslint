...
function findGroupsInTemplateLiteral(node) {
	...
	while (
		hasNext(quasisIndex, node.quasis) ||
		hasNext(expressionsIndex, node.expressions)
	) {
		...
		if (isFirstInSourceCode(currentQuasi, currentExpression)) {
			// handle quasi
			let { hasMorePortions, portionOfInterest, indexOfNextPortion } =
				extractPortionOfInterest(currentQuasi, quasiTextIndex);

			quasiTextIndex = indexOfNextPortion;

			if (portionOfInterest)
				pushToCurrentGroup(portionOfInterest, groups);
...
function extractPortionOfInterest(currentQuasi, quasiTextIndex) {
	...
	let portionOfInterest = currentQuasiValue.substring(
		quasiTextIndex,
		endOfPortionOfInterest
	);
...
function prettyPrintNonSingletonGroup(group, sourceCode) {
	return group.map((part) => prettyPrintGroupItem(part, sourceCode)).join("");
}
