...
function resolveInlinableLiterals(node, context) {
	return (fixer) => {
		...
		let inlinedString = inlined.map(({ key }) => `"${getObjectKeyValue(key)}"`)
			.join(", ");

		let othersString = others.map((property) => sourceCode.getText(property))
			.join(", ");
...
function handleArrayExpression(node, context) {
	...
	let shouldBeInlined = node.elements.every((element) => element.type === "Literal")
...
function handleObjectExpression(node, context) {
	...
	node.properties.forEach(({ key: propertyKeyNode, value: propertyValueNode }) => {
