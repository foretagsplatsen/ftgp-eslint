...
module.exports = function(context) {
		...
		if (jsdocNode) {
			...
			Object.keys(specParams).forEach(function(p) {
				if (paramsToCheck.indexOf(p) === -1) {
					context.report({
						node: jsdocNode,
						message: 'Non-matching JSDoc parameter \'{{name}}\'.',
						data: {
							name: p
						}
					});
	...
	return {
		...
		'ExpressionStatement': function(node) {
			...
			var fnParams = right.params;
			if (!fnParams[0]) {
				return;
			}

			var spec;
			if (fnParams[0].type === 'ObjectPattern') {
				spec = fnParams[0];
			} else if (fnParams[0].type === 'AssignmentPattern') {
				spec = fnParams[0].left;
			} else {
				return;
			}

			spec.properties.forEach(function(property) {
				params.push('spec.' + property.key.name);
			});
		},
